...
import com.cqjtu.cms.exception.BeanUtilsException;
...
/**
 * BeanUtils Utils for bean operation.
 *
 * @author johnniang
 * @date 11/22/18
 */
public class BeanUtils {
  ...
  public static <T> List<T> transfromInBatch(final Class<T> clazz, Collection<?> sources) {
    return CollectionUtils.isEmpty(sources)
        ...
        : sources.stream().map(source -> transfrom(clazz, source)).collect(Collectors.toList());
  }
